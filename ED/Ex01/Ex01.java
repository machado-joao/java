...
public class Ex01 {
	...
	public static void main (String[] args) {
		...
		for(int i = 0; i < ordenadoPorValores.length; i++) {
			System.out.printf("(%d) %s (%d)\n", i + 1, ordenadoPorValores[i].getNome(), ordenadoPorValores[i].getTotal());
		}
		...
		for(int i = 0; i < ordenadoPorValoresPresencial.length; i++) {
			System.out.printf("(%d) %s (%d)\n", i + 1, ordenadoPorValoresPresencial[i].getNome(), ordenadoPorValoresPresencial[i].getTotal());
		}
		...
		for(int i = 0; i < rankingPontosPessoa.size(); i++) {
			System.out.printf("(%d) %s (%d)\n", i + 1, rankingPontosPessoa.get(i).getNome(), rankingPontosPessoa.get(i).getPontos());
		}
